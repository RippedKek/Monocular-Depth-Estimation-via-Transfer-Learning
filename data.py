...
class NYU_BasicAugmentRGBSequence(Sequence):
    ...
    def __getitem__(self, idx, is_apply_policy=True):
        ...
        for i in range(batch_x.shape[0]):
            ...
            # Resolve possible relative paths robustly (support running from repo root)
            def resolve_path(p):
                # If already absolute or exists as-is, return it
                ...
                if os.path.exists(candidate):
                    return candidate
                # try parent of cwd (legacy '../' usage)
                candidate = os.path.join(os.getcwd(), '..', p)
                if os.path.exists(candidate):
                    return candidate
                # fallback to original path (will raise later)
                return p

            # If the image is stored inside the extracted zip (self.data), load from bytes
            from io import BytesIO
            if sample[0] in self.data:
                x_img = Image.open(BytesIO(self.data[sample[0]]))
            else:
                x_img = Image.open(resolve_path(sample[0]))

            if sample[1] in self.data:
                y_img = Image.open(BytesIO(self.data[sample[1]]))
            else:
                y_img = Image.open(resolve_path(sample[1]))

            x = np.clip(np.asarray(x_img).reshape(480,640,3)/255,0,1)
            y = np.clip(np.asarray(y_img).reshape(480,640,1)/255*self.maxDepth,0,self.maxDepth)
...
class NYU_BasicRGBSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):            
            ...
            sample = self.dataset[index]
            # Resolve paths (see NYU_BasicAugmentRGBSequence.resolve_path)
            def resolve_path(p):
                if os.path.isabs(p) and os.path.exists(p):
                    return p
                if os.path.exists(p):
            ...
            # If the image is inside the extracted zip (self.data), load from bytes first
            from io import BytesIO
            if sample[0] in self.data:
                x_img = Image.open(BytesIO(self.data[sample[0]]))
            else:
            ...
            x = np.clip(np.asarray(x_img).reshape(480,640,3)/255,0,1)
            y = np.asarray(y_img, dtype=np.float32).reshape(480,640,1).copy().astype(float) / 10.0
            y = DepthNorm(y, maxDepth=self.maxDepth)
