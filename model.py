...
def create_model(existing='', is_twohundred=False, is_halffeatures=True):
    ...
    if len(existing) == 0:
        ...
        def upproject(tensor, filters, name, concat_with):
            ...
            # Resolve layer name in base_model robustly (support both 'conv1/relu' and 'conv1_relu' etc.)
            def resolve_layer(layer_name):
                try:
                    return base_model.get_layer(layer_name)
                except ValueError:
                    # try common alternative: replace '/' with '_'
                    alt = layer_name.replace('/', '_')
                    try:
                        return base_model.get_layer(alt)
                    except ValueError:
                        # try matching by suffix (last token after '/'), if unique
                        token = layer_name.split('/')[-1]
                        matches = [l for l in base_model.layers if l.name.endswith(token)]
                        if len(matches) == 1:
                            return matches[0]
                        # try contains prefix and suffix
                        if '/' in layer_name:
                            prefix = layer_name.split('/')[0]
                            for l in matches:
                                if prefix in l.name:
                                    return l
                        # not found -> re-raise a helpful error
                        raise ValueError(f"No such layer: {layer_name}. Existing layers are: {[l.name for l in base_model.layers]}")

            target_layer = resolve_layer(concat_with)
            up_i = Concatenate(name=name+'_concat')([up_i, target_layer.output]) # Skip connection
