...
def get_nyu_callbacks(model, basemodel, train_generator, test_generator, test_set, runPath):
    ...
    # Callback: save checkpoints every N epochs (replacement for deprecated `period` arg)
    class PeriodicModelCheckpoint(keras.callbacks.Callback):
        def __init__(self, filepath, period=1, monitor='val_loss', verbose=0, save_weights_only=False):
            super().__init__()
        ...
        def on_epoch_end(self, epoch, logs=None):
            ...
            if (epoch + 1) % self.period == 0:
                ...
                try:
                    ...
                    filename = self.filepath.format(epoch=epoch+1, val_loss=val)
                except Exception:
                    # fallback if formatting fails
                    filename = self.filepath.replace('{epoch:02d}', f"{epoch+1:02d}")
    ...
    callbacks.append(PeriodicModelCheckpoint(runPath + '/weights.{epoch:02d}-{val_loss:.2f}.hdf5', period=5, verbose=1, save_weights_only=False))
